<?php
	...
	function registra_recargas() {

		$labels = array(
			'name'                  => _x( 'Recargas', 'Recargas', 'text_domain' ),
			'singular_name'         => _x( 'Recargas', 'Recargas', 'text_domain' ),
			'menu_name'             => __( 'Recargas', 'text_domain' ),
			'name_admin_bar'        => __( 'Recargas', 'text_domain' ),
		...
		register_post_type( 'Recargas', $args );

	}

	add_action( 'init', 'registra_recargas' );

	function cwp_register_taxonomy_recargas(){
		register_taxonomy(
		'recargas',
		array('recargas'),
		array(
		'label' => __('Recargas'),
		'labels' => array(),
		'rewrite' => array('slug' => 'recargas'),
		'hierarchical' => true
		));
		}
add_action('init', 'cwp_register_taxonomy_recargas');